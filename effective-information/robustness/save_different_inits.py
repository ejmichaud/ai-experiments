...
for realization, init in enumerate(inits): 
    print("Starting realization {} of {} using {}".format(realization+1, NUM_REALIZATIONS, init))
    
    model = SoftmaxRegression(28*28, 10).to(device)
    model.apply(weight_initializer(init))
    loss_fn = nn.NLLLoss()
    ...
    for epoch in range(80):
        for sample, target in train_loader:
            ...
            if num_batches % 100 == 0:
                update_metrics()
    print("Appending data for initializer {}".format(init))
    graph_data[init] = (num_batches_data, eis_data, losses_data, accuracies_data)
    
import pickle
with open("plots/graph_data_different_inits.pkl", "wb") as f:
