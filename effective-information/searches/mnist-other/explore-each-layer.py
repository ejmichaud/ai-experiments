...
from foresight.ei import ei, get_shapes
...
dataset_path = dir_path.parent.parent.parent / "data/mnist.pkl.gz"
...
topologies = {
    ...
    '784 -> 100 -> 100 -> 100 -> 100 -> 10': [28*28, 100, 100, 100, 100, 10],
    '784 -> 20 -> 20 -> 20 -> 10': [28*28, 20, 20, 20, 10]
}
...
def generate_data(topology, use_bias, activation, initializer):
    ...
    accuracies_data = []
    layer_eis_data = {n:[] for n, _ in enumerate(model.layers)}
    ...
    for epoch in range(80):
        for sample, target in train_loader:
            ...
            if num_batches % 50 == 0:
                with torch.no_grad():
                    EI = ei(model, shapes=shapes, device=device)
                    for i, layer in enumerate(model.layers):
                        layer_ei = ei(layer, shapes={list(shapes)[i]: shapes[list(shapes)[i]]}, device=device)
                        layer_eis_data[i].append(layer_ei)
                        
                outof = 0
                loss = 0
                with torch.no_grad():
                    for x, labels in islice(test_loader, 0, 200): # 200 batches of 20 samples
                ...
                loss = 0
                with torch.no_grad():
                    for x, labels in islice(train_loader, 0, 200): # 200 batches of 20 samples
                        output = model(x.to(device))
                        loss += loss_fn(output, labels.to(device)).item()
                        _, pred = torch.max(output, 1)
                        outof += len(labels)
    ...
    data = (params, (num_batches_data, eis_data, layer_eis_data, losses_data, accuracies_data))
