...
inits = [(0, 1), (0, 2.5), (0, 5), (0, 10), (0, 20), (5, 1), (5, 2.5), (5, 5), (5, 10), (5, 20)]

NUM_REALIZATIONS = len(inits)

def weight_initializer(mean, std):
    def init_weights(m):
...
for realization, (mean, std) in enumerate(inits): 
    print("Starting realization {} of {} using N({}, {})".format(realization+1, NUM_REALIZATIONS, mean, std))
    
    model = SoftmaxRegression(28*28, 10).to(device)
    model.apply(weight_initializer(mean, std))
    loss_fn = nn.NLLLoss()
    ...
    for epoch in range(80):
        for sample, target in train_loader:
            ...
            if num_batches % 100 == 0:
                update_metrics()
    print("Appending data for initializer N({}, {})".format(mean, std))
    graph_data[(mean, std)] = (num_batches_data, eis_data, losses_data, accuracies_data)
    
import pickle
with open("plots/graph_data_different_inits2.pkl", "wb") as f:
